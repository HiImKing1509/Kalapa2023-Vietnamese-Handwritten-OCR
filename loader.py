...
class alignCollate(object):
    ...
    def __call__(self, batch):
        ...
        imgH = self.imgH
        imgW = self.imgW
        images = [resizePadding(image, self.imgW, self.imgH)
                  for image in images]
...
class DatasetLoader(object):
    def __init__(self, root, train_file, test_file, imgW, imgH):
        ...
        self.train_dataset = ImageFileList(
            root, self.train_file, transform=img_loader, target_transform=target_loader)

        self.test_dataset = ImageFileList(
            root, self.test_file, transform=img_loader, target_transform=target_loader)
