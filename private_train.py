...
parser.add_argument('--valInterval', type=int, default=50,
...
if opt.pretrained != '':
    ...
    pretrain = torch.load(opt.pretrained, map_location='cuda:0')
...
for epoch in range(1, opt.nepoch+1):
    t = tqdm(iter(train_loader), total=len(
        train_loader), desc='Epoch {}'.format(epoch))
    ...
    if epoch % opt.saveInterval == 0:
        torch.save(
            crnn.state_dict(), f'{opt.expr_dir}/pretrain_{opt.pretrained[5:-4]}_{epoch}.pth')
