...
class BeamSearchDecoder():
    def __init__(self, lib, corpus, chars, word_chars, beam_width=20, lm_type='Words', lm_smoothing=0.01, tfsess=None):
        ...
        self.beamsearch_decoder = word_beam_search_module.word_beam_search(
            self.mat, beam_width, lm_type, lm_smoothing, corpus, chars, word_chars)
...
def cer_loss_one_image(sim_pred, label):
    loss = Levenshtein.distance(sim_pred, label) * \
        1.0 / max(len(sim_pred), len(label))
    return loss
